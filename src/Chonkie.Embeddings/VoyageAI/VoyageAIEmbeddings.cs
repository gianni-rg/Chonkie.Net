...
namespace Chonkie.Embeddings.VoyageAI
{
    ...
    public class VoyageAIEmbeddings : BaseEmbeddings
    {
        ...
        public override async Task<float[]> EmbedAsync(string text, CancellationToken cancellationToken = default)
        {
            ...
            catch (HttpRequestException ex)
            {
                throw new EmbeddingNetworkException(
                    $"Network error occurred while calling Voyage AI embeddings API: {ex.Message}",
                    ex);
            }
            catch (TaskCanceledException ex)
            {
                throw new EmbeddingNetworkException(
                    "Request to Voyage AI embeddings API timed out",
                    ex);
            }
            catch (JsonException ex)
            {
                throw new EmbeddingInvalidResponseException(
                    $"Failed to parse Voyage AI API response: {ex.Message}",
                    ex);
            }
            catch (KeyNotFoundException ex)
            {
                throw new EmbeddingInvalidResponseException(
                    "Voyage AI API response missing expected 'data' or 'embedding' property",
                    ex);
            }
            ...
            catch (Exception ex)
            {
                throw new EmbeddingException(
                    $"Unexpected error during Voyage AI embedding: {ex.Message}",
                    ex);
            }
        }
        ...
        public override async Task<IReadOnlyList<float[]>> EmbedBatchAsync(IEnumerable<string> texts, CancellationToken cancellationToken = default)
        {
            ...
            {
                throw new EmbeddingException(
                    $"Unexpected error during Voyage AI batch embedding: {ex.Message}",
                    ex);
            }
        }
    }

...
internal static class TestHelpers
{
    ...
    /// Uses CHONKIE_SENTENCE_TRANSFORMER_MODEL_PATH only; no hard-coded fallback path.
    /// </summary>
    /// <returns>The model path to use for tests, or empty string if not set.</returns>
    public static string GetModelPath()
    {
        var envPath = Environment.GetEnvironmentVariable("CHONKIE_SENTENCE_TRANSFORMER_MODEL_PATH");
    ...
    {
        if (!IsModelAvailable())
            Assert.Skip("CHONKIE_SENTENCE_TRANSFORMER_MODEL_PATH not set or model.onnx not found. Skipping test that requires ONNX model.");
    }
}
